...
int altaAvion(eAviones avion[], int tam, int id, eViaje viajes[],int tamViaje, eMarca marcas[], int tamMarca) {
	...
	if (pos == -1) {
		...
		while (auxAvion.matricula < 1000 ) {
							printf("error ingrese una matricula valida (A partir de 1000) )");
							scanf("%d", &auxAvion.matricula);
						}
...
void mostrarAvion(eAviones aviones, eMarca marca[], int tamMar,eViaje viajes[], int tamViaj) {
	...
	printf("%8d   %8d     %15s  %15s    %8d  %10d     %10d/%2d/%d\n", aviones.idAvion,aviones.modelo, descViaje, descMarc, aviones.matricula,aviones.cantAsientos,aviones.fecha.dia,aviones.fecha.mes,aviones.fecha.anio);
...
void modificarAvion (eAviones aviones[], int tamA, eMarca marcas[], int tamMar,eViaje viajes[], int tamVia)
{
    ...
    if(indice == -1)
    {
        printf("No hay registro de la Avion con el Id: %d\n",id);
    }
    else
    {
